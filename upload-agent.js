...
// src/lib/runtime/upload-agent.ts
var upload_agent_exports = {};
__export(upload_agent_exports, {
  UploadAgent: () => UploadAgent
});
module.exports = __toCommonJS(upload_agent_exports);
...
var guardReturn = /* @__PURE__ */ __name(async (promise, timeout = 1e3, errMessage = "action hang guard timed out") => {
...
var UploadAgent = class {
  constructor(config2, mock = false) {
    this.config = config2;
    this.mock = mock;
    this.MAX_UPLOADS = 5;
    this.MONITOR_INTERVAL = 5e3;
    this.UPLOAD_TIMEOUT = 6e5;
    // 10 minutes
    this.assets = [];
    this.reportComplete = false;
    setInterval(this.monitor.bind(this), this.MONITOR_INTERVAL);
  ...
  listenForMessages() {
    process.stdin.on("data", async (data) => {
      ...
      switch (key) {
        case "report":
          this.reportAsset = this.createAsset(
            "report" /* Report */,
            value
          );
          await this.uploadAsset(this.reportAsset);
  ...
  async monitorComplete() {
    if (!this.reportComplete && this.reportAsset && this.reportAsset.complete) {
      this.reportComplete = true;
      console.log("upag:report-uploaded");
    }
    if (this.reportComplete && this.assets.filter((a) => a.complete || a.error).length === this.assets.length) {
      await awaitSleep(100);
      console.log(`upag:complete`);
      return;
    }
   ...
   */
  async startUploadServer() {
    const port = await this.acquirePortNumber();
    console.log(`upag:port=${port}`);
    ...
    this.server = http.createServer((req, res) => {
      ...
      req.on("end", () => {
        ...
        const { path, type, testId } = parsedBody;
        res.writeHead(200, { "Content-Type": "text/plain" });
        if (!path) {
          res.end("Missing path field");
          return;
        }
  ...
  async uploadAsset(asset) {
    console.log("uploading asset", asset.path);
    if (this.mock) {
      asset.response = new Promise((resolve, reject) => {
        setTimeout(() => {
          asset.complete = true;
          console.log("Asset uploaded successfully", asset.path);
    ...
    if (!asset.uploadURL) {
      console.log("Error getting signed URL for asset", asset.path);
      asset.error = true;
      return;
    }
    const stats = (0, import_fs.statSync)(asset.path);
    const fileSizeInBytes = stats.size;
    let readStream = (0, import_fs.createReadStream)(asset.path);
    const res = guardReturn(
      (0, import_node_fetch.default)(asset.uploadURL, {
        method: "PUT",
        headers: {
    ...
    res.then((res2) => {
      ...
      console.log("Error uploading asset", asset.path, error);
      asset.error = true;
    });
    asset.response = res;
  }
  async getSignedURLForUpload(asset) {
    try {
      ...
      console.error("Error getting signed URL for artifact", e);
      return null;
    }
  }
  /**
   ...
   */
  addAsset(type, path, testId) {
    console.log("adding asset", path);
    const asset = this.createAsset(type, path, testId);
    this.assets.push(asset);
    return asset;
  }
  createAsset(type, path, testId) {
    return {
      path,
      type,
      testId,
  ...
  /**
   * Returns path for temporary assets file for test session
   */
  //   getTestSessionTempAssetsPath(): string {
  //     return path.join(
  //       getTestDataPath(this.baseDir),
  //       `${this.sessionId}.temp.json`
  //     );
  //   }
  /**
...
__name(UploadAgent, "UploadAgent");
var config;
try {
  config = JSON.parse(process.argv[2]);
  if (!config.sessionId || !config.apiKey || !config.checksumApiURL) {
    throw new Error("Invalid config");
  }
} catch (e) {
  console.log("Error starting upload agent", e);
  process.exit(1);
}
var _a;
new UploadAgent(config, ((_a = process.argv) == null ? void 0 : _a[3]) === "mock");
...
mime-types/index.js:
   ...
   *)
*/
//# sourceMappingURL=upload-agent.js.map
